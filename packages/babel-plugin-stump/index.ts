...
function loadNycConfig(cwd: string, opts: { nycrcPath: string }) {
  let memokey = cwd;
  ...
  const result = JSON.parse(execFileSync(process.execPath, args).toString());
...
function findConfig(opts: { cwd: string; nycrcPath: string }) {
    ...
    visitor: {
      Program: {
        enter(path: any) {
          ...
          //TODO monorepo support not skip!
